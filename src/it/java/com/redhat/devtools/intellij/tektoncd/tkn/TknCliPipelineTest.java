...
public class TknCliPipelineTest extends TknCliTest {
    ...
    public void testVerifyCreatePipelineAndDelete() throws IOException {
        ...
        getTkn().deletePipelines(NAMESPACE, pipelines, false);
        pipelines = getTkn().getPipelines(NAMESPACE).stream().map(pp -> pp.getMetadata().getName()).collect(Collectors.toList());;
    ...
    public void testVerifyPipelineYAMLIsReturnedCorrectly() throws IOException {
        ...
        String resourceBodyFromCluster = getTkn().getPipelineYAML(NAMESPACE, PIPELINE_NAME);
    ...
    public void testVerifyCreatePipelineAndPipelineRunAndDeleteAll() throws IOException, InterruptedException {
        ...
        TestUtils.saveResource(getTkn(), pipelineConfig, NAMESPACE, "pipelines");
        TestUtils.saveResource(getTkn(), pipelineRunConfig, NAMESPACE, "pipelineruns");
        ...
        Watch watch = getTkn().getClient(TektonClient.class).v1beta1().pipelineRuns().inNamespace(NAMESPACE)
        ...
        try {
            getTkn().cancelPipelineRun(NAMESPACE, namePipelineRun[0]); //pipelinerun may have already finished its execution
        } catch(IOException ignored){}
        // clean up and verify cleaning succeed
        getTkn().deletePipelines(NAMESPACE, Arrays.asList(PIPELINE_NAME), true);
        pipelines = getTkn().getPipelines(NAMESPACE).stream().map(pp -> pp.getMetadata().getName()).collect(Collectors.toList());;
        assertFalse(pipelines.contains(PIPELINE_NAME));
        List<PipelineRun> pipelineRuns = getTkn().getPipelineRuns(NAMESPACE, PIPELINE_NAME);
        assertTrue(pipelineRuns.stream().noneMatch(run -> run.getMetadata().getName().equals(PIPELINE_RUN_NAME)));
    }

    public void testVerifyStartPipelineCreateRuns() throws IOException, InterruptedException, TimeoutException, ExecutionException {
        ...
        getTkn().startPipeline(NAMESPACE, PIPELINE_NAME, params, "", Collections.emptyMap(), Collections.emptyMap(), "");
        final String[] namePipelineRun = {""};
        Watch watch = getTkn().getClient(TektonClient.class).v1beta1().pipelineRuns().inNamespace(NAMESPACE)
        ...
        // clean up
        getTkn().deletePipelines(NAMESPACE, Arrays.asList(PIPELINE_NAME), true);
        getTkn().deleteTasks(NAMESPACE, Arrays.asList(TASK_NAME), false);
    }
